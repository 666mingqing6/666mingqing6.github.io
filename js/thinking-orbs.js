...
/* 全局挂载（原 ESM export 改为 window 全局，供 classic script 使用） */
...
/* ==========================================================================
 * ThinkingOrb 原生 JS 驱动器
 * 用法：mountThinkingOrb(canvasEl, { state, size, speed, theme, paused })
 * 返回清理函数（停止动画、移除监听）
 * ========================================================================== */
window.mountThinkingOrb = function mountThinkingOrb(canvas, options) {
  const engine = window.ThinkingOrbEngine;
  if (!engine || !canvas) return null;

  const cfg = options || {};
  const state = cfg.state || 'working';
  const size = cfg.size || 64;
  const speed = cfg.speed || 1;
  const theme = cfg.theme || 'auto';
  const paused = !!cfg.paused;

  // 主题解析：暗色背景用浅色墨点，亮色背景用深色墨点
  function isDark() {
    ...
    while (el) {
      ...
      if (el.classList && el.classList.contains('light')) return false;
      el = el.parentElement;
    }
  ...
  canvas.setAttribute('aria-label', 'AI is thinking');

  const ctx = canvas.getContext('2d');
  if (!ctx) return null;

  const preset = engine.resolvePreset(state, size);
  const draw = engine.MODE_DRAWS[preset.mode];
  const rate = preset.speed * speed;

  function paint(now) {
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    ctx.clearRect(0, 0, size, size);
    draw(ctx, size, (now / 1000) * rate, isDark(), preset.opts);
  }

  if (reduced) {
    paint(600); // 减少动态：只画一帧静态代表帧
    return null;
  }
  ...
  function start() {
    if (!running && !paused) { running = true; raf = requestAnimationFrame(tick); }
  }
